...
class SiameseNetworkforTCAM:
    ...
    def _construct_siamese_architecture(self, learning_rate_multipliers,
        ...
        vgg19_net = Sequential()
        vgg19_net.add(Conv2D(filters=32, kernel_size=(3,3),activation='relu',input_shape=self.input_shape, kernel_regularizer=l2(l2_regularization_penalization['Conv1']),name='Conv1'))  
        vgg19_net.add(Conv2D(filters=32, kernel_size=(11,11),activation='relu', kernel_regularizer=l2(l2_regularization_penalization['Conv2']),name='Conv2')) 
        vgg19_net.add(Conv2D(filters=32, kernel_size=(7,7),activation='relu', kernel_regularizer=l2(l2_regularization_penalization['Conv3']),name='Conv3')) 
        vgg19_net.add(MaxPool2D())
            
        vgg19_net.add(Conv2D(filters=64, kernel_size=(5,5),activation='relu', kernel_regularizer=l2(l2_regularization_penalization['Conv4']),name='Conv4'))  
        vgg19_net.add(Conv2D(filters=64, kernel_size=(5,5),activation='relu', kernel_regularizer=l2(l2_regularization_penalization['Conv5']),name='Conv5')) 
        vgg19_net.add(MaxPool2D())
            
        vgg19_net.add(Conv2D(filters=128, kernel_size=(3,3),activation='relu', kernel_regularizer=l2(l2_regularization_penalization['Conv6']),name='Conv6'))    
               
        vgg19_net.add(Flatten())
        vgg19_net.add(Dense(units=1000, activation='sigmoid',kernel_regularizer=l2(l2_regularization_penalization['Dense1']),name='Dense1'))
        vgg19_net.add(Dense(units=100, activation='sigmoid',name='Dense2'))
        # Now the pairs of images
        input_image_1 = Input(self.input_shape)
        input_image_2 = Input(self.input_shape)              

        encoded_image_1 = vgg19_net(input_image_1)
        encoded_image_2 = vgg19_net(input_image_2)
    ...
    def train_siamese_network(self, number_of_iterations, support_set_size,
        ...
        #for iteration in range(36001,number_of_iterations,1):
            ...
            if (iteration + 1) %10000 ==0:   # for memory clear
                cur_iter=str(iteration+1)
                model_json = self.model_test.to_json()
                if not os.path.exists('./models'):
                    os.makedirs('./models')
                ...
                self.model_test.save_weights('models/' + model_name+'_iter_'+cur_iter + '.h5')
                K.clear_session()
                self.model_train.load_weights('./models/'+model_name+'_iter_'+cur_iter+'.h5')
            ...
            if iteration - best_accuracy_iteration > 10000:
                ...
                print('best_eval Accuracy = ' + str(best_eval_accuracy))
                break
