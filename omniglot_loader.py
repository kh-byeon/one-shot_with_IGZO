    ...
    def one_shot_test(self, model, support_set_size, number_of_tasks_per_alphabet,
        ...
        for alphabet in alphabets:
            ...
            for _ in range(number_of_tasks_per_alphabet): # 40
                ...
                #tmp=np.array(images)
                #print('tm',tmp.shape)
                dist_btw_embbeding = model.predict_on_batch(images)
                #print('min',np.argmin(dist_btw_embbeding))
                #tmp1=np.array(dist_btw_embbeding)
                #print('pred',tmp1.shape)
                #print('tmp1',dist_btw_embbeding)
        ...
        #print('len alpha',len(alphabets))
        #print('#',number_of_tasks_per_alphabet)
