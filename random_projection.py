...
class RandomProj:
    ...
    def _construct_siamese_architecture(self, learning_rate_multipliers,
        ...
        flat=tf.keras.layers.Flatten()(input_image_1)
        flat=tf.expand_dims(flat,axis=-1)
        # Let's define the cnn architecture
        gaus=tf.random.normal((10,11025),mean=0,stddev=1)
        matmul_layer=Lambda(lambda tensors : tf.matmul(gaus,tensors[0]))
        random_proj=matmul_layer(flat)
        """
        input_image = Input(self.input_shape)
        output_image = input_image
        self.model_no_proj=Model(inputs=input_image, outputs=output_image)
        """
        ...
        self.model_proj = Model(inputs=input_image_1, outputs=random_proj)
        ...
        self.model_proj.compile(loss=self.contrastive_loss_w_margin(self.margin),metrics=['kullback_leibler_divergence']  , optimizer='SGD')
